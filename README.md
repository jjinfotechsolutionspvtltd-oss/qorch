...
```bash
pip install -e ".[dev]" && python -m pytest      # 668 tests, ~40s, no services required
```
...
- **Correct by construction** — immutable IR, 668 tests, mypy-clean, with property and cross-simulator validation.
...
tests/                   # 668 unit tests (~95% coverage)
```
