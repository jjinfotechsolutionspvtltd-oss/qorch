...
```bash
pip install -e ".[dev]" && python -m pytest      # 634 tests, ~40s, no services required
```
...
- **Correct by construction** — immutable IR, 634 tests, mypy-clean, with property and cross-simulator validation.
...
tests/                   # 634 unit tests (~95% coverage)
```

## Tests

```bash
python -m pytest                 # 634 tests
