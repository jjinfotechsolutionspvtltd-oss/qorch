...
```bash
pip install -e ".[dev]" && python -m pytest      # 825 tests, ~40s, no services required
```
...
- **Correct by construction** — immutable IR, 825 tests, mypy-clean, with property and cross-simulator validation.
...
tests/                   # 825 unit tests (~95% coverage)
```

## Tests

```bash
python -m pytest                 # 825 tests
