...
```bash
pip install -e ".[dev]" && python -m pytest      # 968 tests, ~40s, no services required
```
...
- **Correct by construction** — immutable IR, 968 tests, mypy-clean, with property and cross-simulator validation.
...
tests/                   # 968 unit tests (~95% coverage)
```

## Tests

```bash
python -m pytest                 # 968 tests
