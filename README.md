...
```bash
pip install -e ".[dev]" && python -m pytest      # 805 tests, ~40s, no services required
```
...
- **Correct by construction** — immutable IR, 805 tests, mypy-clean, with property and cross-simulator validation.
...
tests/                   # 805 unit tests (~95% coverage)
```

## Tests

```bash
python -m pytest                 # 805 tests
