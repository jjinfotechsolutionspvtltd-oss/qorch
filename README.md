...
```bash
pip install -e ".[dev]" && python -m pytest      # 692 tests, ~40s, no services required
```
...
- **Correct by construction** — immutable IR, 692 tests, mypy-clean, with property and cross-simulator validation.
...
tests/                   # 692 unit tests (~95% coverage)
```
