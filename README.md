...
```bash
pip install -e ".[dev]" && python -m pytest      # 855 tests, ~40s, no services required
```
...
- **Correct by construction** — immutable IR, 855 tests, mypy-clean, with property and cross-simulator validation.
...
tests/                   # 855 unit tests (~95% coverage)
```
