...
```bash
pip install -e ".[dev]" && python -m pytest      # 792 tests, ~40s, no services required
```
...
- **Correct by construction** — immutable IR, 792 tests, mypy-clean, with property and cross-simulator validation.
...
tests/                   # 792 unit tests (~95% coverage)
```

## Tests

```bash
python -m pytest                 # 792 tests
