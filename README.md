...
```bash
pip install -e ".[dev]" && python -m pytest      # 770 tests, ~40s, no services required
```
...
- **Correct by construction** — immutable IR, 770 tests, mypy-clean, with property and cross-simulator validation.
...
tests/                   # 770 unit tests (~95% coverage)
```

## Tests

```bash
python -m pytest                 # 770 tests
