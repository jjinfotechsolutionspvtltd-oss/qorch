...
```bash
pip install -e ".[dev]" && python -m pytest      # 717 tests, ~40s, no services required
```
...
- **Correct by construction** — immutable IR, 717 tests, mypy-clean, with property and cross-simulator validation.
...
tests/                   # 717 unit tests (~95% coverage)
```

## Tests

```bash
python -m pytest                 # 717 tests
