...
```bash
pip install -e ".[dev]" && python -m pytest      # 618 tests, ~40s, no services required
```
...
- **Correct by construction** — immutable IR, 618 tests, mypy-clean, with property and cross-simulator validation.
...
tests/                   # 618 unit tests (~95% coverage)
```

## Tests

```bash
python -m pytest                 # 618 tests
